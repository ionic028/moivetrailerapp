...
const Moviedetails = () => {
  ...
  return info ? (
    <div
      ...
      className="sm:w-screen sm:h-[160vh] sm:px-[10%] h-[225vh] overflow-y-hidden overflow-x-hidden"
    >
      ...
      <div className="w-full sm:flex relative mb-20 sm:mt-0 mt-10 sm:left-0 left-10 ">
        ...
        <div className="content ml-[5%] sm:mt-0 mt-10 text-white">
          ...
          <div className="  flex text-white items-center sm:gap-x-3 mt-3 mb-10  gap-x-1   ">
            ...
            <h1 className=" w-[60px] font-semibold sm:text-2xl leading-5">User Score</h1>
            <h1 className="sm:text-base text-xs">({info.detail.release_date})</h1>
            <h1  className="sm:text-base text-xs ">{info.detail.genres.map((g,i) => g.name).join(",")}</h1>
            <h1  className="sm:text-base text-xs sm:mr-0 mr-20">{info.detail.runtime}min</h1>
          </div>

          <h1 className="sm:text-xl   font-semibold italic text-zinc-200">{info.detail.tagline}</h1>
          ...
          <h1 className="text-xl mb-3 mt-3">Movie Translated</h1>
          <p className="mb-10 sm:text-base text-xs sm:mr-0 mr-20 ">{info.translations.join(", ")}</p>
          <Link className="absolute  sm:top-20 top-12 right-[60px] sm:right-3 py-5 px-5  bg-[#6556CD] rounded-lg" to={`${pathname}/trailer`}> <i className="text-xl mr-2 ri-play-fill"></i>Play Trailer</Link>
        </div>
      </div>
      ...
      <div className="absolute top-[57%] sm:left-12 left-5   sm:w-[20%] w-[7%] flex flex-col sm:gap-y-5 sm:mt-10 mt-32  " >
        ...
        <h1 className="sm:text-2xl text-xl sm:text-left text-center font-semibold text-white">Recommendations & Similar Stuff</h1>
